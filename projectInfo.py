...
def updateExternals():
  ...
  for t in tools["SETUP"]:
    ...
    if t not in tools["SETUP"]: tools["SETUP"][t] = {}
    tc = tools["SETUP"][t]
    if "USE" in tc:
      for d in tc["USE"]:
  ...
  if os.path.exists(os.path.join(localtop, ".SCRAM", arch, "MakeData", "Tools", "SCRAMBased", "order")):
    for t in check_output('sort -r ${localtop}/.SCRAM/${arch}/MakeData/Tools/SCRAMBased/order', shell=True).decode().rstrip()
      t = re.sub(r'^\d+:', r'', t)
      base = ""
      if t == "self": base = reltop
      else: base = "%s_BASE" % t.upper()
